...
class Export:Sheet {
    ...
    @discardableResult override init() {
        ...
        DispatchQueue.global(qos:.background).async {
            let cgImage = Sharer.export(List.shared.selected.board)
    ...
    private func saveTo(_ url:URL) {
        ...
        (List.shared.selected.board.name as NSString).draw(in:NSRect(x:32, y:390, width:360, height:50), withAttributes:
